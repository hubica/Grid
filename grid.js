...
(function () {
	...
	mark = function (x, y, color) {
		...
		if (typeof x !== 'number') {
			throw new Error('Argument x mÃ¥ste vara en siffra');
		}

		if (typeof y !== 'number') {
			throw new Error('Argument y mÃ¥ste vara en siffra');
		}
		...
		if (color === '#fff' || color === '#ffffff') {
			// normalisera vit sÃ¥ att vit fÃ¤rg alltid betyder unmarked
			color = 'white';
		}
	...
	isMarked = function (x, y) {
		...
		if (typeof grid[x] !== 'undefined' && typeof grid[x][y] !== 'undefined') {
			// ifall debug Ã¤r igÃ¥ng sÃ¥ fÃ¥r vi kolla i kÃ¶n ifall rutan Ã¤r markerad eller inte
			if (queue.length > 0) {
				var foundAtIndex = -1;

				// vi letar upp den senaste ifall man kÃ¶rt mark(1,1);unmark(1,1);mark(1,1); sÃ¥ hittar den att den kommer vara true till slut
	...
	colorOf = function (x, y) {
		...
		// utanfÃ¶r rutnÃ¤tet finns inga fÃ¤rger, ge null
		return null;
	}
